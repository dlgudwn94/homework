...
<!-- - `login.md` 파일에 과제를 구현해 나가는 과정에 대한 설명과 회고가 인상적 임.
다만 코드 컬리링이 다소 아쉬움.
- <h1> 요소 즉 `대제목`이 `누락` 되었음.
- SVG 로고의 대체 텍스트로 “네이버 로고 이미지”를 제공하고 있음. 다만 “로고 이미지”라는 텍스트를 굳이 제공하기보다 `“네이버”`로만 제공하는 것을 추천하고 싶음.
- `컴포넌트 관점`으로 접근하는 사고가 부족해 보임. 아이디 입력 서식의 경우 <label>과 <input> 그리고 에러 메시지 영역까지 하나의 컴포넌트로 `“재활용”`한다면 <div> 요소 등을 활용하여 그룹화 하는 것이 필요함.
- <label> 요소를 적절하게 잘 사용하였으나 해당 요소에 `텍스트 레이블(예-아이디)이 누락`되어 빈 레이블만 제공하고 있음. 이 경우 스크린 리더는 입력서식인 <input> 요소에 어떤 정보를 입력해야하는지 알 수 없어 사용자는 혼란을 느끼게 됨.
- `로그인 버튼의 마크업 순서`는 디자인 순서와 같이 중간이 아닌 마지막에 마크업하여 사용자가 콘텐츠를 접근하는 순서가 논리적이 될 수 있도록 하는 것을 추천 함.
- <label> 요소에 강제로 tabindex=”0”을 제공하였기 때문에 키보드로 접근은 가능하지만 `조작이 불가능` 함. 결국 키보드를 이용하여 해당 서식을 조작할 수 없음.
- <a> 요소로 마크업 한 IP 보안의 경우 target 속성의 값이 “_blank”라면 `rel` 속성을 함께 제공하여야 하며 해당 속성의 값으로 `noopener`와 `noreferrer`가 필요함.
- ON/OFF는 스위치 UI로 `<input type=”checkbox” />`로 마크업 한 후 CSS를 사용하여 스위치 토글 기능을 구현하여야 함.
- Custom Properties를 잘 활용하였으나 —green-color: #03cf5d;를 —primary-color: var(—green-color) 형식으로 `변수의 값을 다른 변수에 재할당`하는 방법을 추천 함.
- CSS의 가상 클래스 선택자를 활용하면 입력 값이 유효한지에 다라 스타일링을 다르게 적용할 수 있음. (:vaild, :invaild, `:user-vaild`, `:user-invaild` 등) -->
